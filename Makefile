...
start_db:
	...
	docker run --rm -d --name partition-mysql \
            -p 3306:3306 \
            -v $(DB_DIR):/var/lib/mysql \
            -e MYSQL_DATABASE=partition_test \
...
rm_db:
	docker stop partition-mysql || true
	rm -Rf $(DB_DIR)

refresh_db: rm_db start_db

get_ip:
	@docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' partition-mysql
